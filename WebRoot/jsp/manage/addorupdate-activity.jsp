...
<form action="" method="post" class="definewidth m20" id="form_submit">
    ...
    <table class="table table-bordered table-hover m10">
        <tr>
            ...
            <td><input type="text" name="activity.activityTitle" id="activityTitle" value="${activity.activityTitle}"/>
            </td>
            <td rowspan="3" class="tableleft" style="text-align:center;vertical-align:middle;"><span
                    style="color: red;">* </span>图片<br><span style="color: red;">建议尺寸:640x180</span></td>
            <td rowspan="3">
        ...
        <tr>
            ...
            <td>
                <input type="text" name="activity.activityName" id="activityName" value="${activity.activityName}"/>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <c:if test="${activity.status=='1'|| activity.status!='0' }">
                    <input type="radio" name="activity.status" value="1" checked/> 启用
                    <input type="radio" name="activity.status" value="0"/> 无效
                </c:if>
                <c:if test="${activity.status=='0' }">
                    <input type="radio" name="activity.status" value="1"/> 启用
                    <input type="radio" name="activity.status" value="0" checked/> 无效
                </c:if>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="3"><textarea id="activityLink" style="resize: none;width: 500px;" rows="3"
                                      name="activity.activityLink">${activity.activityLink }</textarea></td>
        </tr>
        <tr>
            <td class="tableleft" style="text-align:center;vertical-align:middle;">介绍</td>
            <td colspan="3"><textarea style="resize: none;width: 500px;" rows="7"
                                      name="activity.activityIntro">${activity.activityIntro }</textarea></td>
        </tr>
        <tr>
            ...
            <td>
                <c:choose>
                    <c:when test="${activity.validDate=='1' }">
                        <input type="radio" name="activity.validDate" value="0" onclick="onclickRadio()"> 限时有效
                        <input type="radio" name="activity.validDate" value="1" checked="checked"
                               onclick="onclickRadio()"> 长期有效(不需要设置开始结束时间)
                    </c:when>
                    <c:otherwise>
                        <input type="radio" name="activity.validDate" value="0" checked="checked"
                               onclick="onclickRadio()"> 限时有效
                        <input type="radio" name="activity.validDate" value="1"
                               onclick="onclickRadio()"> 长期有效(不需要设置开始结束时间)
                    </c:otherwise>
                </c:choose>
            </td>
        ...
        <tr>
            ...
            <td><s:select list="list_channel" listKey="channel_id" listValue="channel_name" id="list_role"
                          name="activity.channelId" value="activity.channelId" headerKey="0"
                          headerValue="--选择渠道--"/></td>
        ...
        <tr>
            <td class="tableleft" style="text-align:center;vertical-align:middle;">所属商家</td>
            <td><s:select list="listPartners" listKey="id" listValue="partners_name" name="activity.partnersId"
                          value="activity.partnersId" headerKey="0" headerValue="--选择商家--"/></td>
            <td class="tableleft" style="text-align:center;vertical-align:middle;"><span style="color: red;">* </span>权重
            </td>
            <td><input type="text" name="activity.weight" id="weight" value="${activity.weight }"
        ...
        <tr>
            ...
            <td colspan="3"><textarea style="resize: none;width: 500px;" rows="7" cols="30" name="activity.remark"
                                      id="remark">${activity.remark }</textarea></td>
        </tr>
        <tr>
...
<script>
    ...
    function submitForm() {
        ...
        $.ajax({
            url: "${path}/manage/addOrupdateActivity.action",
            type: 'POST',
            data: data,
            dataType: 'JSON',
            cache: false,
            ...
            if (flag === "ok") {
                alert(ret.msg);
                window.location.href = "${path}/jsp/manage/list_activity.jsp";
            } else if (flag === "false") {
                alert(ret.msg);
            } else {
                alert("操作失败");
    ...
    function checkData() {
        ...
        var validDate = $("input[name='activity.validDate']:checked").val();
        if (validDate == 0) {
