...
<form class="form-inline definewidth m20" action="index.html" method="get">
    ...
    上架状态： <s:select list="#{2:'已上线',0:'已下架',1:'等待上架确认'}" listKey="key" cssStyle="width:130px" listValue="value"
    ...
    <button type="button" class="btn btn-success" id="addParentMenu" onclick="toAddOrUpdatePage(0)">新增卡券</button> &nbsp;
</form>
<div id="right">
...
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.msg.list.length; i++) {
                    ...
                    if (obj[18] == '0') {
                        strs = strs + "<td><a style='cursor:pointer;color: red;' onclick='updateTicketPutaway(0,1," + obj[0] + ")'>已下架</a></td>";//销售状态
                    } else if (obj[18] == '1') {
                        strs = strs + "<td><a style='cursor:pointer;color: red;' onclick='updateTicketPutaway(1,0," + obj[0] + ")'>等待上架确认</a></td>";//销售状态
                    } else if (obj[18] == '2') {
                        strs = strs + "<td><a style='cursor:pointer;' onclick='updateTicketPutaway(2,0," + obj[0] + ")'>已上线</a></td>";//销售状态
                    } else {
                    ...
                    if (obj[11] == "1") {
                        ...
                        strs = strs + "<td><a style='cursor:pointer;' onclick='delOrUpdate(0," + obj[0] + "," + data.msg.extendsObj + ")'>删除</a>|<a style='cursor:pointer;'onclick='toAddOrUpdatePage(" + obj[0] + ")'>查看/编辑</a></td></tr>";
                    } else {
                        strs = strs + "<td style='color: red;'>无效</td>";
                        strs = strs + "<td>" + obj[12] + "</td>";
                        if (data.msg.extendsObj == null || data.msg.extendsObj == '0' || data.msg.extendsObj == '') {
                            strs = strs + "<td><a style='cursor:pointer;' onclick='delOrUpdate(1," + obj[0] + "," + data.msg.extendsObj + ")'>启用</a>|<a style='cursor:pointer;'onclick='toAddOrUpdatePage(" + obj[0] + ")'>查看/编辑</a></td></tr>";
                        } else {
                            strs = strs + "<td style='color: red;'>请联系管理员恢复卡券</td></tr>";
                        }
    ...
    function delOrUpdate(status, id, extendsObj) {
        ...
        $.ajax({
            ...
            async: false,
            cache: false,
            url: "${path}/manage/deleteTicketStatus.action",
            dataType: "json",
            success: function (data) {
                if (data.flag == "ok") {
                    alert(data.msg);
    ...
    function toAddOrUpdatePage(id) {
        if ("0" == id) {//新增
            var op = "add";
            window.location.href = "${path}/manage/toAddTicketPage.action?op=" + op;
        } else {//修改
            var op = "update";
            window.location.href = "${path}/manage/toAddTicketPage.action?op=" + op + "&id=" + id;
        }
    }
    ...
    //更新商品上架状态
    function updateTicketPutaway(nowIsPutaway, isPutaway, goodsId) {
        ...
        $.ajax({
            ...
            async: false,
            cache: false,
            url: "${path}/manage/updateTicketPutaway.action",
            dataType: "json",
            success: function (data) {
                if (data.flag == "ok") {
                    alert(data.msg);
