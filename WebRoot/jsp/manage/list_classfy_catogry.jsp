...
<form class="form-inline definewidth m20" action="index.html" method="get">
    ...
    类目名称： <s:select list="#{0:'--类目名称--'}" listKey="key" cssStyle="width:110px" listValue="value" value=""
                    id="catogryId"/>&nbsp;
    <p></p>
    类目编码： <input id="categoryCode" type="text" class="abc input-default" placeholder="" value="">&nbsp;
...
<script>
    ...
    function listdata(currentPage) {
        ...
        $.ajax({
            ...
            async: false,
            cache: false,
            url: "${path}/manage/loadCatogryLeve1ListPage.action",
            dataType: "json",
            success: function (data) {
                ...
                for (var i = 0; i < data.msg.list.length; i++) {
                    ...
                    //标识0 名称2 编码1	图片7	权重3	类目名称5	状态4	创建时间6
                    strs = strs + "<tr><td>" + obj[0] + "</td>";//编号
                    strs = strs + "<td>" + obj[2] + "</td>";//名称
                    strs = strs + "<td>" + obj[1] + "</td>";//编码
                    strs = strs + "<td><img width='35px' height='35px' style='border: solid 1px;border-color: black;' src='" + obj[7] + "'></td>";
                    strs = strs + "<td>" + obj[3] + "</td>";//权重
                    strs = strs + "<td>" + obj[5] + "</td>";//类目名称
                    if (obj[4] == "1") {
                        strs = strs + "<td >有效</td>";
