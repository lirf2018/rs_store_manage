...
<form class="form-inline definewidth m20" action="index.html" method="get">
    bannel名称： <input id="bannelName" type="text" class="abc input-default" placeholder="" value="">&nbsp;
    ...
    渠道： <s:select list="#{0:'--选择渠道--'}" listKey="key" cssStyle="width:110px" listValue="value" value=""
                  id="channel_id"/>&nbsp;
    <p></p>
    bannel状态： <s:select list="#{1:'长期有效',2:'未开始',3:'已开始(未结束)',4:'已结束',5:'限时有效'}" listKey="key" listValue="value"
                        headerKey="0" headerValue="--选择bannel状态--" value="" cssStyle="width:150px" id="objId"/>&nbsp;
    ...
    <button type="button" class="btn btn-success" id="addParentMenu" onclick="toAddOrUpdatePage(0)">新增bannel</button>
    &nbsp;
</form>
<div>
    <table class="table table-bordered table-hover definewidth m10">
        <tr>
            <td><span style="color: red;">main页默认查询6个bannel</span></td>
        </tr>
    </table>
</div>
<div id="right">
...
<script>
    ...
    function listdata(currentPage) {
        ...
        var objId = $("#objId").val();//上架状态
        var status = $("#status").val();//状态
        $.ajax({
            type: "post",
            data: {
                bannelName: bannelName,
            ...
            async: false,
            cache: false,
            url: "${path}/manage/loadbannelDataList.action",
            dataType: "json",
            success: function (data) {
                ...
                for (var i = 0; i < data.msg.list.length; i++) {
                    ...
                    if (obj[8] == 1) {
                        ...
                        if (obj[10] == 0) {
                            strs = strs + "<td style='color: red;'>" + obj[4] + "已过期</td>";//结束时间
                        } else {
                            strs = strs + "<td>" + obj[4] + "</td>";//结束时间
                        }
                    }
                    strs = strs + "<td>" + obj[7] + "</td>";//商家名称
    ...
    function toAddOrUpdatePage(id) {
        if ("0" == id) {//新增
            var op = "add";
            window.location.href = "${path}/manage/toAddorUpdatebannelPage.action?op=" + op;
        } else {//修改
            var op = "update";
            window.location.href = "${path}/manage/toAddorUpdatebannelPage.action?op=" + op + "&id=" + id;
        }
    }
    ...
    function listChannel() {
        $.ajax({
            ...
            success: function (data) {
                var msg = data.msg;
                var str = "<option value='' selected='selected'>--选择渠道--</option>";
                if (msg.length > 0) {
