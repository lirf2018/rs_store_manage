...
<form action="" method="post" class="definewidth m20" id="form_submit">
    ...
    <div id="right">
        <table class="table table-bordered table-hover definewidth m10">
            <tr>
                <td>商品名称：<input id="ticketName" type="text" value="">
...
<script>
    ...
    //查询规则列表
    function listRule() {
        var goodsId = $("#relId").val();
        $.ajax({
            type: "post",
            data: {ruleName: $("#ruleName").val(), goodsId: goodsId},
            async: false,
            cache: false,
            url: "${path}/manage/getAddGoodsRulePageListRule.action",
            dataType: "json",
    ...
    function listTicket() {
        $.ajax({
            ...
            async: false,
            cache: false,
            url: "${path}/manage/getAddGoodsRulePageListGoods.action",
            dataType: "json",
            success: function (data) {
                var obj = data.msg;
                $("#tb_ticket").empty();
                ...
                if (obj.length != 0) {
                    for (var i = 0; i < obj.length; i++) {
                        str = "<tr style='cursor:pointer;' onclick='onclickRelRadio(this)'><td><input  value='" + obj[i].goods_id + "' type='radio' name='rel.relId' ></td><td>" + obj[i].goods_name + "</td></tr>";
    ...
    function submitForm() {
        ...
        //上传完得到id 或者路径 标志上传成功
        $.ajax({
            url: "${path}/manage/saveGoodsRule.action",
            type: 'POST',
            ...
            success: function (data) {
                ...
                if (flag === "ok") {
                    alert(data.msg);
                    window.location.href = "${path}/jsp/manage/list_goodsrule.jsp";
                } else if (flag === "false") {
                    alert(data.msg);
                } else {
                    alert("操作失败");
