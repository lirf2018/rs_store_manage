...
<form class="form-inline definewidth m20" action="index.html" method="get">
    行政区划代码： <input style="width: 100px" id="regionCode" type="text" name="regionCode" class="abc input-default"
                   placeholder="" value="">&nbsp;
    行政区划名称： <input id="regionName" type="text" name="regionName" class="abc input-default" placeholder="" value="">&nbsp;
    上级行政区划代码： <input style="width: 100px" id="regionCodeP" type="text" name="regionCodeP" class="abc input-default"
                     placeholder="" value="">&nbsp;<span id="regionCodePName" style="color: grey"></span>&nbsp;<br><br>
    状态： <s:select list="#{1:'有效',0:'无效'}" listKey="key" listValue="value" headerKey="" headerValue="--全部--" value=""
                  cssStyle="width:80px" id="status"/>&nbsp;
    ...
    <br/><br/>
    地区层级： <s:select list="#{1:'省/自治区/直辖市/特别行政区',2:'市/省(自治区)直辖县/省直辖区/自治州',3:'市辖区/县/自治县',4:'乡/镇/街道',5:'村'}" listKey="key"
                    listValue="value" headerKey="-1" headerValue="--全部--" value="" cssStyle="width:180px" id="level"/>&nbsp;
...
<script>
    ...
    function listdata(currentPage) {
        ...
        $.ajax({
            ...
            data: {
                ...
                regionCodeP: regionCodeP,
                level: level,
                currentPage: currentPage
            },
            async: false,
            cache: false,
            url: "${path}/manage/loadAddrManagePage.action",
            dataType: "json",
            success: function (data) {
                $("#regionCodePName").empty();
                $("#regionCodePName").append(data.msg.extendsString);
                //清空保存的数据
                $("#tb").empty();
    ...
    function delOrUpdate(status, id) {
        $.ajax({
            ...
            async: false,
            cache: false,
            url: "${path}/manage/updateRegionStatus.action",
            dataType: "json",
            success: function (data) {
                if (data == 1) {
                    alert("操作成功");
    ...
    function toAddOrUpdatePage(id) {
        ...
        if ("0" == id) {//新增
            var op = "add";
            window.location.href = "${path}/manage/toUpdateRegionPage.action?op=" + op;
        } else {//修改
            var op = "update";
            window.location.href = "${path}/manage/toUpdateRegionPage.action?op=" + op + "&id=" + id;
        }
    }
