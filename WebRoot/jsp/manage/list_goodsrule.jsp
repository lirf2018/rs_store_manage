...
<form class="form-inline definewidth m20" action="" method="get">
    商品名称： <input id="goodsName" type="text" name="goodsName" class="abc input-default" placeholder="" value="">&nbsp;
    规则名称： <input id="ruleName" type="text" name="ruleName" class="abc input-default" placeholder="" value="">&nbsp;
    商家名称： <input id="partnersName" type="text" name="partnersName" class="abc input-default" placeholder="" value="">&nbsp;<p></p>
    店铺名称： <input id="shopName" type="text" name="shopName" class="abc input-default" placeholder="" value="">&nbsp;
    规则时效： <s:select id="timeLimit" list="#{1:'长期有效',2:'未开始',3:'已开始(未结束)',4:'已结束',5:'限时有效'}" listKey="key"
                    listValue="value" headerKey="-1" headerValue="--选择时效--" value="" cssStyle="width:140px"/>
    <button type="button" class="btn btn-primary" onclick="listdata(1,3)">查询</button> &nbsp;
    <button type="button" class="btn btn-success" id="addParentMenu" onclick="toAddOrUpdatePage()">新增商品规则</button>
    &nbsp;
</form>
...
<script>
    ...
    function listdata(currentPage) {
        ...
        $.ajax({
            ...
            async: true,
            cache: false,
            url: "${path}/manage/loadGoodsRulePage.action",
            dataType: "json",
            success: function (data) {
                ...
                for (var i = 0; i < data.msg.list.length; i++) {
                    ...
                    strs = strs + "<tr>";
                    strs = strs + "<td>" + obj[0] + "</td>";//规则编号
                    strs = strs + "<td>" + obj[1] + "</td>";//商品名称
                    strs = strs + "<td>" + obj[2] + "</td>";//规则名称
                    ...
                    strs = strs + "<td>" + obj[9] + "</td>";//商家名称
                    strs = strs + "<td>" + obj[8] + "</td>";//店铺名称
                    strs = strs + "<td>" + obj[5] + "</td>";
                    strs = strs + "<td><a style='cursor:pointer;' onclick='delOrUpdate(" + obj[0] + ")'>删除</a></td></tr>";
                    $("#tb1").append(strs.toString());
                }
                ...
                if (data.msg.list.length > 0) {
                    ...
                    $("#tb1").append("<tr><td colspan='20' style='font-size: 20px;font-weight: bold;color: red'>暂时没有数据</td></tr>");
                }
            }
        });
    }
    ...
    function toAddOrUpdatePage() {
        window.location.href = "${path}/manage/toAddGoodsRuleRel.action";
    }
