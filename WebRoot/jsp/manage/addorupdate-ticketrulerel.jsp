...
<script>
    ...
    //查询规则列表
    function listRule() {
        var ticketId = $("#relId").val();
        $.ajax({
            type: "post",
            data: {ruleName: $("#ruleName").val(), ticketId: ticketId},
            async: false,
            cache: false,
            url: "${path}/manage/getAddTicketRulePageListRule.action",
            dataType: "json",
    ...
    function listTicket() {
        $.ajax({
            ...
            async: false,
            cache: false,
            url: "${path}/manage/getAddTicketRulePageListTicket.action",
            dataType: "json",
            success: function (data) {
                var obj = data.msg;
                $("#tb_ticket").empty();
                ...
                if (obj.length != 0) {
                    for (var i = 0; i < obj.length; i++) {
                        str = "<tr style='cursor:pointer;' onclick='onclickRelRadio(this)'><td><input  value='" + obj[i].tikcet_id + "' type='radio' name='rel.relId' ></td><td>" + obj[i].tikcet_name + "</td></tr>";
    ...
    function submitForm() {
        ...
        //上传完得到id 或者路径 标志上传成功
        $.ajax({
            url: "${path}/manage/saveTicketRule.action",
            type: 'POST',
            ...
            success: function (data) {
                ...
                if (flag === "ok") {
                    alert(data.msg);
                    window.location.href = "${path}/jsp/manage/list_ticketrule.jsp";
                } else if (flag === "false") {
                    alert(data.msg);
                } else {
                    alert("操作失败");
