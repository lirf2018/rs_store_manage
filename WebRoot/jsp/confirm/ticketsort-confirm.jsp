...
<form class="form-inline definewidth m20" action="index.html" method="get">
    ...
    <button type="button" class="btn btn-success" onclick="paramConfirm()">卡券排序确认</button> &nbsp;
</form>
<div id="right">
...
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.msg.list.length; i++) {
                    ...
                    strs = strs + "<td><input type='text' data-id='" + obj[0] + "' value='" + obj[6] + "' style='width: 45px;text-align: center' name='ticketSort'></td>";//排序
    ...
    function paramConfirm() {
        ...
        //处理选中的数据
        var values = "";
        var ticketSort = $("input[name=ticketSort]");
        var flag = false;
        var sort = "";
        ticketSort.each(function () {
            sort = $(this).val() + "";
            sort = sort.trim();
            if ('' == sort) {
                //非法排序
        ...
        $.ajax({
            type: "post",
            data: {values: values, paramKey: paramKey},
            async: false,
            cache: false,
            url: "${path}/manage/ticketsortConfirm.action",
            dataType: "json",
            success: function (data) {
                if (data.flag == "ok") {
                    alert(data.msg);
