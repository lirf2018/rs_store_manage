...
<form action="" method="post" class="definewidth m20" id="submit_form">
    <input type="hidden" value="subMenu" id="menuType" name="menuType"/>
    <input type="hidden" value="${function.functionId}" id="functionId" name="function.functionId"/>
    <table class="table table-bordered table-hover m10">
        <tr>
            <td class="tableleft" style="text-align:center;vertical-align:middle;"><span style="color: red;">* </span>父级菜单名称
            </td>
            <td><s:select id="functionParentid" name="function.functionParentid" list="listMenu" listKey="function_id"
                          listValue="function_name" value="function.functionParentid" headerKey="0"
                          headerValue="请选择父级菜单"></s:select></td>
            <td class="tableleft" style="text-align:center;vertical-align:middle;"><span style="color: red;">* </span>子菜单名称
            </td>
            <td><input type="text" name="function.functionName" id="functionName" autofocus="autofocus"
                       value="${function.functionName}"/></td>
        </tr>
        <tr>
            <td class="tableleft" style="text-align:center;vertical-align:middle;"><span style="color: red;">* </span>子菜单编码
            </td>
            ...
            <td><input type="text" name="function.sort" id="sort" value="${function.sort }"
                       onkeyup="this.value=this.value.replace(/[^\d]/g,'')"/> <span
                    style="color: red;">只能输入数字,数值越大越靠前</span></td>
        </tr>
        <tr>
            <td class="tableleft" style="text-align:center;vertical-align:middle;"><span style="color: red;">* </span>功能动作
            </td>
            <td colspan="3"><input type="text" name="function.functionAction" id="functionAction" style="width: 500px"
                                   value="${function.functionAction}"/></td>
        </tr>
        <tr>
...
<script>
    ...
    //保存数据
    function submitForm() {
        var parent = $("#functionParentid").val() + "";
        if (parent.trim() == "0") {
            alert("请选择父级菜单");
            return;
        }
        ...
        var functionAction = $("#functionAction").val() + "";
        if (functionAction.trim() == "") {
            alert("请输入功能动作");
            return;
        }
        var sort = $("#sort").val() + "";
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.flag == "ok") {
                    alert("操作成功");
                    window.location.href = "${path}/menu/toMenuListPage.action?pmenuid=" + $("#functionParentid").val();
