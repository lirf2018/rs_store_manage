        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.msg.list.length; i++) {
                    ...
                    if (obj[17] == "1") {
                        strs = strs + "<td >有效</td>";
                        if ("admin" == obj[1] || "super" == obj[1]) {
                            strs = strs + "<td></td>";
// 									strs=strs+"<td><a style='cursor:pointer;' onclick='delOrUpdateAdmin(0,"+obj[0]+")'>删除</a>|<a style='cursor:pointer;'onclick='toAddAdminPage("+obj[0]+",1)'>查看/编辑</a></td>";
                        } else {
                            strs = strs + "<td><a style='cursor:pointer;' onclick='delOrUpdateAdmin(0," + obj[0] + ")'>删除</a>|<a style='cursor:pointer;'onclick='toAddAdminPage(" + obj[0] + ",1)'>查看/编辑</a></td>";
                        }
                    } else if (obj[17] == "0") {
                        ...
                        if ("admin" == obj[1] || "super" == obj[1]) {
                            strs = strs + "<td></td>";
// 									strs=strs+"<td><a style='cursor:pointer;' onclick='delOrUpdateAdmin(1,"+obj[0]+")'>启用</a>|<a style='cursor:pointer;'onclick='toAddAdminPage("+obj[0]+",1)'>查看/编辑</a></td>";
                        } else {
                            strs = strs + "<td><a style='cursor:pointer;' onclick='delOrUpdateAdmin(1," + obj[0] + ")'>启用</a>|<a style='cursor:pointer;'onclick='toAddAdminPage(" + obj[0] + ",1)'>查看/编辑</a></td>";
                        }
                    } else if (obj[17] == "3") {
...
// 									strs=strs+"<td><a style='cursor:pointer;' onclick='delOrUpdateAdmin(1,"+obj[0]+")'>启用</a>|<a style='cursor:pointer;'onclick='toAddAdminPage("+obj[0]+",1)'>查看/编辑</a></td>";
    ...
    //跳转到增加或者修改管理员页面
    function toAddAdminPage(adminid, type) {
        if (type == 0) {
            handleType = "add";
            window.location.href = "${path}/user/toAddAdminPage.action?handleType=" + handleType;
        } else {
            handleType = "update";
            window.location.href = "${path}/user/toAddAdminPage.action?handleType=" + handleType + "&admin_id=" + adminid;
        }
    }
